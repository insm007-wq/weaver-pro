...
// 이름으로 script/reference 각각 최신 1개 반환 (없으면 null)
ipcMain.handle("prompts:getPairByName", async (_e, name) => {
  try {
    ...
    return { ok: true, data: { script: pick("script"), reference: pick("reference") } };
  } catch (e) {
    return { ok: false, message: String(e?.message || e) };
  }
});

// 두 카테고리를 한 번에 저장/생성해서 원자적으로 반영
ipcMain.handle("prompts:savePair", async (_e, payload) => {
  try {
    const { name, scriptContent, referenceContent } = payload || {};
    const nm = (name || "").trim();
    if (!nm) return { ok: false, message: "name is required" };
    ...
    return { ok: true, data: { script: s || null, reference: r || null } };
  } catch (e) {
    return { ok: false, message: String(e?.message || e) };
  }
});
  ...
  return [
    {
      ...
      name: "튜토리얼 영상",
      description: "단계별 설명이 포함된 튜토리얼 영상용 프롬프트",
      category: "script",
      variables: ["topic", "duration", "difficulty"],
      template: `{topic}에 대한 {duration}분 길이의 {difficulty} 수준 튜토리얼 영상 대본을 작성해주세요.

구성:
1. 인트로 (10% - 주제 소개 및 학습 목표)
2. 준비 사항 (10% - 필요한 도구나 사전 지식)
3. 단계별 실습 (70% - 상세한 설명과 예시)
4. 마무리 및 요약 (10% - 핵심 포인트 정리)

각 단계마다 시청자가 따라하기 쉽도록 명확하고 구체적인 지시사항을 포함해주세요.`,
      isDefault: true,
      createdAt: Date.now(),
      updatedAt: Date.now(),
    },
    {
      id: "template-review",
      name: "제품 리뷰",
      description: "제품이나 서비스 리뷰 영상용 프롬프트",
      category: "script",
      variables: ["product", "duration", "pros", "cons"],
      template: `{product}에 대한 {duration}분 리뷰 영상 대본을 작성해주세요.

구성:
1. 제품 소개 (15% - 기본 정보 및 첫인상)
2. 장점 분석 (40% - {pros})
3. 단점 및 아쉬운 점 (25% - {cons})
4. 총평 및 추천 여부 (20% - 구매 가이드)

객관적이고 균형잡힌 시각으로 작성하며, 실제 사용 경험을 바탕으로 구체적인 예시를 포함해주세요.`,
      isDefault: true,
      createdAt: Date.now(),
      updatedAt: Date.now(),
    },
    {
      ...
      name: "비교 분석",
      description: "두 제품이나 서비스를 비교하는 영상용 프롬프트",
      category: "script",
      variables: ["item1", "item2", "duration", "criteria"],
      template: `{item1}과 {item2}를 {criteria} 기준으로 비교하는 {duration}분 영상 대본을 작성해주세요.

구성:
1. 비교 개요 (10% - 비교 대상 소개)
2. 각 항목별 비교 (70% - {criteria}별 상세 분석)
3. 종합 비교표 (10% - 장단점 요약)
4. 추천 가이드 (10% - 상황별 추천)

공정하고 객관적인 비교를 위해 동일한 기준을 적용하고, 각각의 특징을 명확히 구분해주세요.`,
      isDefault: true,
      createdAt: Date.now(),
      updatedAt: Date.now(),
    },
